...
const SORT_OPTIONS = [{ key: "name", label: "Tên ca" }];
...
export default function Shifts() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`/api/shifts/${editingId}`, formData, authHeader());
        toast.success("Cập nhật ca thành công!");
      } else {
        await axios.post("/api/shifts", formData, authHeader());
  ...
  const handleDelete = async (id) => {
    if (confirm("Bạn có chắc chắn muốn xóa ca này?")) {
      try {
        await axios.delete(`/api/shifts/${id}`, authHeader());
        toast.success("Đã xóa ca thành công!");
        fetchItems();
      } catch (err) {
  ...
  const filtered = shifts.filter((h) =>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
        <h1 className="text-2xl font-bold text-slate-800 uppercase">QUẢN LÝ CA</h1>
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-base font-semibold text-slate-800">Danh sách ca</h2>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-200 text-slate-600 text-xs uppercase tracking-wider">
              ...
              <th className="py-4 px-4 font-semibold">TÊN CA</th>
              <th className="py-4 px-4 font-semibold">GIỜ BẮT ĐẦU</th>
              <th className="py-4 px-4 font-semibold">GIỜ KẾT THÚC</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {currentItems.map((item, index) => (
              <tr key={item.id} className="hover:bg-slate-50 transition-colors text-sm">
                ...
                <td className="py-4 px-4 text-slate-600 whitespace-nowrap">{item.startTime || "-"}</td>
                <td className="py-4 px-4 text-slate-600 whitespace-nowrap">{item.endTime || "-"}</td>
            ...
            {currentItems.length === 0 && (
              <tr><td colSpan={5} className="py-8 text-center text-slate-500 whitespace-nowrap">Không tìm thấy ca nào.</td></tr>
            )}
          </tbody>
        </table>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-6 pb-4 border-b border-slate-100">
              <h2 className="text-xl font-bold text-slate-800">{editingId ? "Chỉnh Sửa Ca" : "Thêm Ca Mới"}</h2>
            </div>
            <form onSubmit={handleSubmit} className="p-6 pt-0 space-y-4">
              ...
              <div>
                ...
                <input type="time" required value={formData.startTime} onChange={(e) => setFormData({ ...formData, startTime: e.target.value })} className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-100 focus:border-indigo-400 outline-none transition-all text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Giờ kết thúc:</label>
                <input type="time" required value={formData.endTime} onChange={(e) => setFormData({ ...formData, endTime: e.target.value })} className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-100 focus:border-indigo-400 outline-none transition-all text-sm" />
              </div>
